    ...
    prepare(root, filenames) {
        ...
        for (const filename of filenames) {
            ...
            try {
                ...
                if (!info.isFile() || info.size > 16 * 1024 * 1024 || (total += info.size) > 32 * 1024 * 1024)
                    throw new Error('Attachments must be regular files, up to 16 MB each and 32 MB combined.');
                data = Buffer.alloc(info.size + 1);
                let read = 0;
                while (read < data.length) {
                    const count = readSync(fd, data, read, data.length - read, null);
                    if (!count)
                        break;
                    read += count;
                }
            ...
            const state = path.join(root, '.relay');
            mkdirSync(state, { recursive: true, mode: 0o700 });
            if (lstatSync(state).isSymbolicLink())
                throw new Error('Unsafe attachment storage.');
            const folder = path.join(state, 'chat-files');
            ...
            if (existingBytes + total > 128 * 1024 * 1024)
                throw new Error('Conversation attachment storage is full. Start a new chat.');
            staged.push({ id, target, data });
            labels.push(`Attached: ${name} (${data.length} bytes)`);
            if (['.png', '.jpg', '.jpeg', '.webm', '.mp4', '.mov'].includes(ext))
                media.push(`![${name.replace(/[\[\]\n\r]/g, '_')}](${target})`);
        ...
        try {
            ...
            for (const filename of committed) {
                try {
                    unlinkSync(filename);
                }
        ...
        for (const file of staged)
            this.files.set(file.id, file.data);
