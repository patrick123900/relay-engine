...
export function engineEnvironment(source, bridgeToken) {
    const result = {};
    for (const [key, value] of Object.entries(source)) {
        if (/KEY|TOKEN|SECRET|PASSWORD|CREDENTIAL|PROVIDER|^RELAY_CHAT_|^OPENAI_|^ANTHROPIC_|^AZURE_|^GOOGLE_|^GEMINI_/i.test(key))
            continue;
        result[key] = value;
    }
...
export function chatConfiguration(env) {
    if (!env.RELAY_CHAT_ENDPOINT || !env.RELAY_CHAT_MODEL)
        return undefined;
    let endpoint;
    try {
        endpoint = new URL(env.RELAY_CHAT_ENDPOINT);
    }
    catch {
        throw new Error("Invalid chat endpoint configuration");
    }
    if (endpoint.username || endpoint.password || endpoint.search || endpoint.hash)
        throw new Error("Chat endpoint cannot contain credentials, query or fragment");
...
export function parameterSchema(parameter) {
    const schema = { type: parameter.type.endsWith("_array") ? "array" : parameter.type };
    for (const key of ["minimum", "maximum", "enum", "pattern", "description", "default"]) {
        ...
        if (parameter[sourceKey] !== undefined)
            schema[key] = parameter[sourceKey];
    }
    if (schema.type === "array") {
        schema.items = { type: parameter.type === "string_array" ? "string" : "number", ...(parameter.pattern ? { pattern: parameter.pattern } : {}) };
        delete schema.pattern;
        if (parameter.minimumLength !== undefined)
            schema.minItems = parameter.minimumLength;
        if (parameter.maximumLength !== undefined)
            schema.maxItems = parameter.maximumLength;
    }
    else if (schema.type === "string") {
        if (parameter.minimumLength !== undefined)
            schema.minLength = parameter.minimumLength;
        if (parameter.maximumLength !== undefined)
...
export class ChatWorkflow {
    ...
    constructor(root, invoke, configuration) {
        this.#root = root;
        this.#invoke = invoke;
        ...
        if (configuration)
            this.#status = "Ready. Agent actions require reviewed native grants.";
    }
    view() {
    ...
    async submit(message, publish, attachmentPaths = []) {
        if (this.#busy)
            throw new Error("Chat workflow is busy");
        this.#abort = new AbortController();
        this.#busy = true;
        let attachments;
        try {
            attachments = this.#attachments.prepare(this.#root, attachmentPaths);
        }
        catch {
            this.#busy = false;
            this.#status = 'Cannot attach files. Choose regular files within the attachment size limits.';
            this.#show('system', this.#status);
            await publish();
            return;
        }
        this.#show("user", (message + "\n" + attachments.display).trim());
        const turn = [{ role: "user", content: attachments.images.length ? [{ type: "text", text: message.slice(0, 4000) + "\n" + attachments.text }, ...attachments.images.map(image => ({ type: "image_url", image_url: { url: image.url } }))] : message.slice(0, 4000) + "\n" + attachments.text }];
        this.#turns.push(turn);
        while (this.#turns.length > 4)
            this.#turns.shift();
        try {
            if (!this.#configuration) {
                this.#status = "Set RELAY_CHAT_ENDPOINT and RELAY_CHAT_MODEL in the external bridge.";
                return;
            }
            ...
            for (let round = 0;; ++round) {
                ...
                if (!automatic && round >= 8)
                    throw new Error("Chat round limit reached");
                if (this.#abort.signal.aborted)
                    throw new Error("Chat stopped");
                ...
                if (Number(response.headers.get("content-length") ?? 0) > 262144) {
                ...
                if (!answer || (answer.content !== null && typeof answer.content !== "string"))
                    throw new Error("Malformed provider message");
                const calls = answer.tool_calls ?? [];
                if (!Array.isArray(calls) || calls.length > 16 || (!automatic && toolCount + calls.length > 16))
                ...
                if (answer.content)
                    this.#show("assistant", answer.content);
                if (!calls.length) {
                    this.#status = "Ready";
                    return;
                }
                ...
                for (const call of calls) {
                    if (this.#abort.signal.aborted || (!automatic && deadline.aborted))
                        throw new Error("Chat stopped");
                    ++toolCount;
                    ...
                    try {
                        if (approvalPending)
                            throw new Error("Waiting for approval; action not executed");
                        if (!method || (!automatic && method.method === "trace.replay") || typeof call.id !== "string" || call.type !== "function")
                            throw new Error("Unsupported tool call");
                        const parameters = JSON.parse(call.function.arguments, (name, value) => {
                        ...
                        succeeded = true;
                        if (method.method === "session.request" && result.pending === true)
                            approvalPending = true;
                    }
                    catch (error) {
                        result = { error: this.#safe(error instanceof Error ? error.message : "Tool failed") };
                    }
