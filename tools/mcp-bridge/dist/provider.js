...
import { chatConfiguration } from "./chat.js";
export function configuration(settings) {
    if (typeof settings.endpoint !== "string" || typeof settings.model !== "string" || typeof settings.credential !== "string" || typeof settings.header !== "string" || settings.endpoint.length > 2048 || settings.model.length > 256 || settings.credential.length > 4096 || settings.header.length > 128)
    ...
    if (!["bearer", "header", "none"].includes(settings.auth))
        throw new Error("Unsupported authentication type");
    if (/[\r\n\x00]/.test(settings.credential))
        throw new Error("Invalid authentication value");
    if (settings.auth === "header" && (!/^[A-Za-z0-9!#$%&'*+.^_`|~-]+$/.test(settings.header) || /^(host|content-length|content-type|connection|cookie|set-cookie|transfer-encoding)$/i.test(settings.header)))
        throw new Error("Invalid authentication header");
    if (result)
        result.authHeader = settings.auth === "header" ? settings.header : "Authorization";
    if (result)
        result.authPrefix = settings.auth === "bearer" ? "Bearer " : "";
    return result;
}
export class ProviderStore {
        ...
        if (existsSync(this.#filename)) {
            ...
            try {
                ...
                if (!info.isFile() || info.nlink !== 1 || info.size > 65536)
                    throw new Error("Unsafe provider configuration file");
                fchmodSync(fd, 0o600);
                ...
                throw new Error("Cannot read private provider configuration; check file format and permissions");
            }
            finally {
                closeSync(fd);
            }
            ...
            if (initial)
                this.#settings = { endpoint: initial.endpoint, model: initial.model, auth: "bearer", header: "", credential: initial.apiKey };
        }
    }
    #checkPaths() {
        for (const filename of [this.#directory, this.#filename]) {
            try {
                ...
                if (info.isSymbolicLink() || (filename === this.#filename && !info.isFile()) || (filename === this.#directory && !info.isDirectory()))
                    throw new Error("Private provider configuration path is unsafe");
            }
            catch (error) {
    ...
    current() { return this.#settings ? configuration(this.#settings) : undefined; }
    view() {
    ...
    save(input) {
        ...
        try {
            writeFileSync(fd, JSON.stringify(settings), "utf8");
            fsyncSync(fd);
        }
        catch (error) {
