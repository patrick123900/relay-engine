...
export interface RpcTransport { request(method: string, params?: JsonObject): Promise<JsonObject>; reply(id: string | number, result: JsonObject): void; onMessage: (method: string, params: JsonObject, id?: string | number) => void; close(): void; }
...
export class CodexTransport implements RpcTransport {
  ...
  constructor(root: string, executable = process.env.RELAY_CODEX_EXECUTABLE ?? "codex") {
    ...
    try { this.#child = spawn(executable, ["app-server", "--stdio", ...overrides.flatMap(value => ["-c", value])], { cwd: path.join(home, "workspace"), env, stdio: ["pipe", "pipe", "pipe"] }); }
    ...
    readline.createInterface({ input: this.#child.stdout }).on("line", line => {
      ...
      if (typeof message.method === "string") {
        ...
        if (message.error) pending.reject(new Error("OpenAI service request failed"));
  ...
  #disconnect(): void {
    ...
    this.#pending.clear(); this.onMessage("relay/disconnected", {});
  }
  close(): void { this.#disconnect(); this.#child.stdin.end(); this.#child.kill("SIGTERM"); }
}
export class OpenAiHarness {
  ...
  async #start(): Promise<void> {
    ...
    const transport = this.#factory(); this.#transport = transport;
  ...
  async #control(control: Control): Promise<void> {
    try {
      ...
      if (control.action === "new_chat") { if (this.#threadId && this.#transport) await this.#transport.request("thread/unsubscribe", { threadId: this.#threadId }); this.#threadId = ""; this.#turnId = ""; this.#messages = []; this.#results = []; this.#status = this.#account ? "Ready" : "Sign in with ChatGPT to start a conversation."; return; }
      await this.#start();
      if (control.action === "signin") {
        ...
        await this.#transport!.request("account/logout"); this.#login = null; this.#account = null; this.#threadId = ""; this.#messages = []; this.#results = []; this.#status = "Signed out.";
  ...
  async #event(method: string, params: JsonObject, id?: string | number): Promise<void> {
    if (method === "relay/disconnected") { this.#transport = undefined; this.#login = null; this.#status = "OpenAI service disconnected. Reconnect to continue."; this.#completion?.(); return; }
    ...
    const matches = this.#busy && params.threadId === this.#threadId && (!this.#turnId || params.turnId === this.#turnId || (params.turn as JsonObject | undefined)?.id === this.#turnId);
    if (id !== undefined) {
      ...
      try {
        ...
        result = await this.#invoke(tool.method, params.arguments as JsonObject); succeeded = true;
  ...
  async submit(message: string, publish: () => Promise<void>): Promise<void> {
    ...
    try {
      ...
      this.#messages.push({ id: `user-${++this.#serial}`, role: "user", content: message.slice(0, 4000).toWellFormed() }); this.#trim();
  ...
  cancel(): void { this.#stopped = true; void this.#interrupt().catch(() => { this.#status = "Stop failed. Reconnect to end the session."; this.close(); }); }
  close(): void { this.#closed = true; this.#stopped = true; this.#completion?.(); this.#transport?.close(); this.#transport = undefined; }
}
